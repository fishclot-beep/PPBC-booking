...
type Session = { id: string; title: string; note?: string | null; starts_at: string; ends_at: string; capacity: number; price_type: "private" | "per_person"; price_amount: number; booked_seats: number; actual_collected: number | null; closed_at: string | null; closed_by_name?: string | null };
...
function AdminDashboard() {
  ...
  const [closure, setClosure] = useState({ startHour: "20", endHour: "22", title: "", note: "", capacity: "0", priceType: "per_person", priceAmount: "" });
  ...
  useEffect(() => { if (!accessDenied) void fetch("/api/admin/maintenance/optimize", { method: "POST" }); }, [accessDenied]);
  ...
  const createClosure = async (event: FormEvent) => {
    ...
    try {
      ...
      await request("/api/admin/sessions", { method: "POST", body: JSON.stringify({
        title: closure.title, note: closure.note, startsAt: `${selectedDate}T${closure.startHour}:00:00+08:00`, endsAt,
  ...
  const updateSession = async (data: Record<string, unknown>) => {
    const updated = await request<Session>("/api/admin/sessions", { method: "PATCH", body: JSON.stringify(data) });
    const replace = (items: Session[]) => items.map((item) => item.id === updated.id ? { ...item, ...updated } : item);
    setRules(replace); setMonthRules(replace); setAllSessions(replace); setEditingSession(null); setNotice("場次內容已更新。");
  };
  ...
  const closeSession = async (session: Session) => { const value = Number(collectedAmounts[session.id] ?? ""); if (!Number.isFinite(value) || value < 0) { setError("請輸入有效的實收金額。"); return; } const updated = await request<Session>("/api/admin/sessions", { method: "PATCH", body: JSON.stringify({ id: session.id, actualCollected: value, collectorId: collectors[session.id] }) }); setMonthRules((current) => current.map((item) => item.id === session.id ? { ...item, ...updated } : item)); setAllSessions((current) => current.map((item) => item.id === session.id ? { ...item, ...updated, closed_by_name: collectors[session.id] ? admins.find((admin) => admin.id === collectors[session.id])?.display_name : item.closed_by_name } : item)); setNotice("此場次已結案。 "); };
  const reopenSession = async (session: Session) => { const updated = await request<Session>("/api/admin/sessions", { method: "PATCH", body: JSON.stringify({ id: session.id, reopen: true }) }); setAllSessions((current) => current.map((item) => item.id === session.id ? { ...item, ...updated, closed_by_name: null } : item)); setNotice("場次已取消結案，已回到待結案。 "); };
  ...
  return <main className={styles.page}>
    ...
    {tab === "venue" && <section className={styles.gridTwo}>
      ...
      <section className={styles.panel}><div className={styles.panelTitle}><div><p>BOOKING SESSIONS</p><h2>{selectedDate} 預約場次</h2></div><span className={styles.statusPill}>{closedHours} 個場次</span></div><p className={styles.help}>預設沒有可預約時段。請在月曆選日期後，建立會員可報名的場次。</p><form className={styles.closureForm} onSubmit={createClosure}><label className={styles.field}>場次名稱<input required placeholder="例如：週二揪團" value={closure.title} onChange={(event) => setClosure({ ...closure, title: event.target.value })} /></label><label className={styles.field}>備註（會員可見）<input maxLength={500} placeholder="例如：請自備深色球衣，提前 10 分鐘集合" value={closure.note} onChange={(event) => setClosure({ ...closure, note: event.target.value })} /></label><div className={styles.timeFields}><label className={styles.field}>開始<select value={closure.startHour} onChange={(event) => setClosure({ ...closure, startHour: event.target.value })}>{Array.from({ length: 15 }, (_, index) => index + 8).map((hour) => <option value={String(hour).padStart(2, "0")} key={hour}>{String(hour).padStart(2, "0")}:00</option>)}</select></label><label className={styles.field}>結束<select value={closure.endHour} onChange={(event) => setClosure({ ...closure, endHour: event.target.value })}>{[...Array.from({ length: 15 }, (_, index) => index + 9), 24].map((hour) => <option value={String(hour).padStart(2, "0")} key={hour}>{hour === 24 ? "00:00（隔日）" : `${String(hour).padStart(2, "0")}:00`}</option>)}</select></label></div><label className={styles.field}>人數上限（0 代表不限）<input required type="number" min="0" max="30" value={closure.capacity} onChange={(event) => setClosure({ ...closure, capacity: event.target.value })} /></label><div className={styles.timeFields}><label className={styles.field}>收費方式<select value={closure.priceType} onChange={(event) => setClosure({ ...closure, priceType: event.target.value })}><option value="per_person">單人費用</option><option value="private">包場費用</option></select></label><label className={styles.field}>金額（元）<input type="number" min="0" value={closure.priceAmount} onChange={(event) => setClosure({ ...closure, priceAmount: event.target.value })} /></label></div><button className={styles.primaryButton}>新增可預約場次</button></form><div className={styles.ruleList}><h3>{selectedDate} 的已開放場次</h3>{rules.length === 0 ? <p className={styles.help}>尚未建立場次，會員無法預約。</p> : rules.map((rule) => <div className={styles.ruleRow} key={rule.id}><div><strong>{rule.title}・{formatTime(rule.starts_at)}–{formatTime(rule.ends_at)}</strong><small>{rule.note ? `${rule.note}・` : ""}{rule.capacity === 0 ? "不限人數" : `${rule.booked_seats}/${rule.capacity} 人`}・{rule.price_type === "private" ? "包場" : "單人"} NT$ {rule.price_amount}</small></div><button type="button" onClick={() => void removeRule(rule)}>刪除場次</button></div>)}</div></section>
    </section>}
    {tab === "venue" && <section className={styles.panel}><div className={styles.panelTitle}><div><p>UPCOMING SESSIONS</p><h2>近 7 天開放場次</h2></div></div><div className={styles.ruleList}>{upcomingSessions.length === 0 ? <p className={styles.help}>近 7 天尚未開放場次。</p> : upcomingSessions.map((session) => <div className={styles.ruleRow} key={`upcoming-${session.id}`}><div><strong>{new Intl.DateTimeFormat("zh-TW", { timeZone: "Asia/Taipei", month: "numeric", day: "numeric", weekday: "short" }).format(new Date(session.starts_at))}・{session.title}</strong><small>{formatTime(session.starts_at)}–{formatTime(session.ends_at)}・{session.capacity === 0 ? "不限人數" : `${session.booked_seats}/${session.capacity} 人`}</small></div><div><button type="button" onClick={() => setEditingSession(session)}>編輯</button><button type="button" onClick={() => void viewReservations(session)}>查看預約</button></div></div>)}</div></section>}
    ...
    {editingSession && <SessionEditor key={editingSession.id} session={editingSession} onCancel={() => setEditingSession(null)} onSave={updateSession} />}
...
function SessionEditor({ session, onCancel, onSave }: { session: Session; onCancel: () => void; onSave: (data: Record<string, unknown>) => Promise<void> }) {
  const inputTime = (value: string) => new Intl.DateTimeFormat("sv-SE", { timeZone: "Asia/Taipei", year: "numeric", month: "2-digit", day: "2-digit", hour: "2-digit", minute: "2-digit", hour12: false }).format(new Date(value)).replace(" ", "T").slice(0, 16);
  const [form, setForm] = useState({ title: session.title, note: session.note ?? "", startsAt: inputTime(session.starts_at), endsAt: inputTime(session.ends_at), capacity: String(session.capacity), priceType: session.price_type, priceAmount: String(session.price_amount) });
  const [error, setError] = useState("");
  const save = async (event: FormEvent) => { event.preventDefault(); setError(""); try { await onSave({ id: session.id, title: form.title, note: form.note, startsAt: `${form.startsAt}:00+08:00`, endsAt: `${form.endsAt}:00+08:00`, capacity: Number(form.capacity), priceType: form.priceType, priceAmount: Number(form.priceAmount) }); } catch (issue) { setError(issue instanceof Error ? issue.message : "儲存失敗。"); } };
  return <section className={styles.panel}><div className={styles.panelTitle}><div><p>EDIT SESSION</p><h2>編輯場次</h2></div><button type="button" onClick={onCancel}>取消</button></div><form className={styles.closureForm} onSubmit={save}><label className={styles.field}>場次名稱<input required maxLength={80} value={form.title} onChange={(event) => setForm({ ...form, title: event.target.value })} /></label><label className={styles.field}>備註（會員可見）<input maxLength={500} value={form.note} onChange={(event) => setForm({ ...form, note: event.target.value })} /></label><div className={styles.timeFields}><label className={styles.field}>開始時間<input required type="datetime-local" value={form.startsAt} onChange={(event) => setForm({ ...form, startsAt: event.target.value })} /></label><label className={styles.field}>結束時間<input required type="datetime-local" value={form.endsAt} onChange={(event) => setForm({ ...form, endsAt: event.target.value })} /></label></div><label className={styles.field}>人數上限（0 代表不限）<input required type="number" min="0" max="30" value={form.capacity} onChange={(event) => setForm({ ...form, capacity: event.target.value })} /></label><div className={styles.timeFields}><label className={styles.field}>收費方式<select value={form.priceType} onChange={(event) => setForm({ ...form, priceType: event.target.value as "private" | "per_person" })}><option value="per_person">單人費用</option><option value="private">包場費用</option></select></label><label className={styles.field}>金額（元）<input required type="number" min="0" value={form.priceAmount} onChange={(event) => setForm({ ...form, priceAmount: event.target.value })} /></label></div>{session.booked_seats > 0 && <p className={styles.help}>目前已有 {session.booked_seats} 人預約；人數上限不可低於此數量。</p>}{error && <p className={styles.error}>{error}</p>}<button className={styles.primaryButton}>儲存修改</button></form></section>;
}

function Operations({ sessions, admins, values, collectors, setValues, setCollectors, closeSession, reopenSession, expectedAmount }: { sessions: Session[]; admins: Admin[]; values: Record<string, string>; collectors: Record<string, string>; setValues: (value: Record<string, string>) => void; setCollectors: (value: Record<string, string>) => void; closeSession: (session: Session) => void; reopenSession: (session: Session) => void; expectedAmount: (session: Session) => number }) {
...
function SettlementHistory({ sessions, reopenSession, expectedAmount }: { sessions: Session[]; reopenSession: (session: Session) => void; expectedAmount: (session: Session) => number }) {
  const closed = sessions.filter((session) => session.closed_at); const stamp = (value: string) => new Intl.DateTimeFormat("zh-TW", { timeZone: "Asia/Taipei", month: "numeric", day: "numeric", hour: "2-digit", minute: "2-digit", hour12: false }).format(new Date(value));
  return <section className={styles.panel}><div className={styles.panelTitle}><div><p>SETTLEMENT DETAIL</p><h2>已結案收款紀錄</h2></div>{closed.length > 0 && <a className={styles.primaryButton} href="/api/admin/reports/closed-sessions">下載 Excel</a>}</div><div className={styles.ruleList}>{closed.length === 0 ? <p className={styles.help}>尚無已結案場次。</p> : closed.map((session) => <div className={styles.settlementRow} key={`history-${session.id}`}><div><strong>{stamp(session.starts_at)}・{session.title}</strong><small>應收 NT$ {expectedAmount(session)}・實收 NT$ {session.actual_collected}・收款人：{session.closed_by_name ?? "未記錄"}</small></div><button className={styles.primaryButton} type="button" onClick={() => reopenSession(session)}>取消結案</button></div>)}</div></section>;
}

function ReservationAudit({ sessions }: { sessions: Session[] }) {
  const [selected, setSelected] = useState(""); const [events, setEvents] = useState<{ id: string; display_name: string; seats: number; event_type: string; created_at: string }[]>([]);
  useEffect(() => { if (!selected) { setEvents([]); return; } void fetch(`/api/admin/sessions/${selected}/history`).then((response) => response.json()).then(setEvents); }, [selected]);
  const stamp = (value: string) => new Intl.DateTimeFormat("zh-TW", { timeZone: "Asia/Taipei", hour: "2-digit", minute: "2-digit", hour12: false }).format(new Date(value));
  return <section className={styles.panel}><div className={styles.panelTitle}><div><p>BOOKING ACTIVITY</p><h2>會員預約異動紀錄</h2></div></div><label className={styles.field}>選擇待結案場次<select value={selected} onChange={(event) => setSelected(event.target.value)}><option value="">請選擇場次</option>{sessions.map((session) => <option value={session.id} key={session.id}>{session.title}・{new Intl.DateTimeFormat("zh-TW", { timeZone: "Asia/Taipei", month: "numeric", day: "numeric" }).format(new Date(session.starts_at))}</option>)}</select></label><div className={styles.ruleList}>{selected && events.length === 0 ? <p className={styles.help}>此場次尚無預約紀錄。</p> : events.map((event) => <div className={styles.ruleRow} key={event.id}><div><strong>{event.display_name}・{event.event_type === "created" ? "首次預約" : event.event_type === "cancelled" ? "取消預約" : "更改人數"}</strong><small>{stamp(event.created_at)}・{event.seats} 人</small></div></div>)}</div></section>;
}
