...
export async function PATCH(request: Request) {
  try { const admin = await requireAdmin(); await ensureSessionSchema(); const { id, actualCollected, collectorId, reopen } = await request.json(); if (typeof id !== "string") return NextResponse.json({ error: "場次資料無效。" }, { status: 400 }); if (reopen === true) { const [session] = await db()`UPDATE booking_sessions SET actual_collected=NULL, closed_at=NULL, closed_by=NULL WHERE id=${id} RETURNING *`; return NextResponse.json(session); } if (typeof actualCollected !== "number" || actualCollected < 0) return NextResponse.json({ error: "實收金額無效。" }, { status: 400 }); const collector = typeof collectorId === "string" ? collectorId : admin.id; const [manager] = await db()`SELECT id FROM members WHERE id=${collector} AND role='admin'`; if (!manager) return NextResponse.json({ error: "請選擇有效的管理人員。" }, { status: 400 }); const [session] = await db()`UPDATE booking_sessions SET actual_collected=${actualCollected}, closed_at=now(), closed_by=${collector} WHERE id=${id} RETURNING *`; if (!session) return NextResponse.json({ error: "找不到場次。" }, { status: 404 }); return NextResponse.json(session); } catch (error) { return fail(error); }
}
