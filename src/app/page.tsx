...
function Home() {
  ...
  return <><main className="app-shell"><header className="topbar"><Link className="brand" href="/">◒ PPBC 籃球俱樂部</Link><nav><Link className="active" href="/">立即預約</Link><Link href="/my-bookings">我的預約</Link><Link href="/admin">管理後台</Link></nav></header><section className="hero"><p className="eyebrow">ONLINE BOOKING</p><h1>選擇場次，<em>立即報名。</em></h1><p>顯示未來 7 天、由管理員開放的預約場次。</p></section><section className="booking-card"><h2>近期可預約場次</h2><div className="session-list">{items.length === 0 ? <div className="empty-selection">未來 7 天尚未開放任何預約場次。</div> : items.map((session) => { const remaining = session.capacity === 0 ? 5 : Math.max(0, session.capacity - session.booked_seats); const count = seats[session.id] ?? 1; return <article className="session-card" key={session.id}><div><p className="eyebrow">{stamp(session.starts_at)}</p><h2>{session.title}</h2><span>{session.capacity === 0 ? "不限人數" : `剩餘 ${remaining}/${session.capacity} 名`}・{session.price_type === "private" ? "包場" : "單人"}費用 {money(Number(session.price_amount))}</span></div><div className="session-actions"><label>預約人數<select value={count} onChange={(event) => setSeats({ ...seats, [session.id]: Number(event.target.value) })}>{Array.from({ length: Math.min(5, remaining) }, (_, index) => <option key={index} value={index + 1}>{index + 1} 人</option>)}</select></label><button className="confirm" disabled={!remaining} onClick={() => void book(session.id)}>{remaining ? "立即預約" : "名額已滿"}</button></div></article>; })}</div></section></main>{notice && <div className="success-modal"><section><p>預約成功</p><h2>已為您保留名額</h2><span>{notice}</span><button className="confirm" onClick={() => setNotice("")}>知道了</button></section></div>}</>;
}
